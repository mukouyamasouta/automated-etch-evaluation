...
# 元の20250525FasterRCNN_TrainingV163.pyは変更しない。
...
# ハイパーパラメータ
EPOCH_NUMBER = 1       # 30 → 1: 最後まで動くかだけを短時間で確認する。
BATCH_SIZE = 1         # 8 → 1: 16GB Macでのメモリ使用量を抑える。
LEARNING_RATE = 0.01
SHUFFLE = True
NUM_WORKERS = 0        # 4 → 0: macOSの別プロセス読込みによる不安定さを避ける。
PIN_MEMORY = False     # True → False: CUDAを使用しないため固定メモリは不要。
DROP_LAST = False      # True → False: 少数データを捨てずにすべて使う。
# 分類における信頼度の閾値
score_threshold = 0.9
# 通常画像かセグメンテーション済み画像か
use_segmented = False
...
# 実験条件を出力と一緒に残し、後からGit差分と対応付けられるようにする。
run_config = {
...
# %% モデル設定
# モデルをロード（事前学習済み）。
# Faster R-CNNは内部でも画像をリサイズするため、Mac版ではmin/maxを明示して計算量を抑える。
...
# デバイス設定。再現性とTorchVision Detectionの互換性を優先し、Mac版はCPU固定。
