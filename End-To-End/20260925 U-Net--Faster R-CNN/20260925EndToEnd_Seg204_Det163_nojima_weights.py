# 野島氏のDet163 epoch 30 / Seg204 epoch 87の学習済み重みで手法BをEnd-to-End実行する。
# 元コードは20250526EndToEnd_Seg204_Det163_1.pyとして保持し、このファイルでは
# CPU MacとCUDA GPUの両方で重みを読み込めるよう実行環境だけを調整している。
# # %% モジュール＆パッケージ
import os
import shutil
import json
...
# 長時間処理の開始前に、重みと入力CSVの配置ミスを検出する。
required_paths = (
    det_model_path,
    seg_model_path,
    DATASET_DIR / "Detection" / "Original" / "test.csv",
)
...
# %% モデル設定
# モデル構造
# 直後にDet163のstate_dictを読み込むため、COCO重みの不要なダウンロードを行わない。
...
model.to(device)
# 学習済みモデルのロード
model.load_state_dict(torch.load(det_model_path, map_location=device, weights_only=True))
print(
    f"Runtime: device={device}, num_workers={NUM_WORKERS}, "
    f"pin_memory={PIN_MEMORY}"
)
print(f"Detection weight: {det_model_path}")
print(f"Segmentation weight: {seg_model_path}")
...
# %% 推論（テスト）
def test(model_path, test_loader, criterion, epoch, history, results_dir, use_seg_annotations, device):
