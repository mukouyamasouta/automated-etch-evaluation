...
│       └── 20250526EndToEnd_Seg204_Det163_1.py
├── Dataset/                         # 実データ。Git管理外
└── docs/
...
バックアップにある多数の旧版`.py.7z`、過去の出力、全epochの`.pth`は管理対象にしていません。手法Bの基準となる組合せは **Seg204 / Det163** です。2本の学習スクリプトと1本のEnd-To-Endスクリプトは、必要なモデル定義とデータ読込み処理を内部に持っています。
...
### 3. 手法BをEnd-To-Endで評価する

Det163の30 epoch目とSeg204の87 epoch目の重みが生成されていることを確認してから実行します。

```bash
python "End-To-End/20260925 U-Net--Faster R-CNN/20250526EndToEnd_Seg204_Det163_1.py"
```

このコードはDetectionのtest.csvに含まれる9枚を対象に、物体検出、構造切出し、U-Net、寸法計測、正解との比較を順番に行います。出力は同じフォルダ内の`InferenceSeg204_Det163_1_outputs/`へ作成され、Git管理から除外されます。
...
3. `.7z`から実行可能な`.py`を展開。
4. CSV内の画像・マスクパスも、スクリプト位置を基準に解決。
